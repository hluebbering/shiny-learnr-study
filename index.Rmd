...
The server function is a set of instructions that build the application’s computational components and run complex R code behind the scenes. In this study, the server function acts as a random number generator to randomize participants into groups $A$ or $B$. The function runs a reactive expression containing an R statement, which creates a random uniform variable. If the uniform variable is greater than $0.5$, A will be the output; otherwise, B will be the output. The last line of the function makes sure that the created random variable will not suspend when hidden.
...
The learner package can turn any R Markdown document into a collaborative solution for facilitating statistics and R programming tutorials. Building on R Markdown, the learnr package enables interactive tutorials featuring various interactive Shiny components. Users can access these interactive tutorials from their browser, via the same mechanisms as a Shiny application. The code exercises created in the learner package display as R code blocks; the user can edit and execute them directly. Progress is automatically preserved, allowing users to save and keep track of completed exercises or questions. 

In addition to supporting interactive tutorials, the learnr package includes built in tools for checking and reinforcing students’ understanding. Users are free to adjust code-snippets, complete coding exercises, read hints and solutions, and execute their code. In the context of this study, we utilize the learnr package as a tool for hosting online experiments designed to examine best practices for teaching statistical or programming concepts.
